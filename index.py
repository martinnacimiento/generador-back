...
@app.route('/dam', methods=['POST'])
def simulationDam():
    try:
        req = request.get_json()
        flow = int(req['flow'])
        capacityEnergy = int(req['capacityEnergy'])
        limitsGate = req['limitsGate']
        flowTurbine = int(req['flowTurbine'])
        limitAlert = int(req['limitAlert'])
        drainAmount = int(req['drainAmount'])
        vars = req['vars']
        resultado = SimulationDam.simulation(vars, flow, drainAmount, capacityEnergy, limitsGate, limitAlert)
        return json.jsonify(resultado)
    except Exception as e:
        return json.jsonify(
            {'msg': "Lo sentimos, hubo un error."})
